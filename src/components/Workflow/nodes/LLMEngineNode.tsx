...
export const LLMEngineNode = ({ data, selected }: any) => {
  ...
  const [prompt, setPrompt] = useState(data.config?.prompt || 'You are a helpful PDF assistant...');
  ...
  const handlePromptChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (data.onUpdate) {
      data.onUpdate(data.id || data.nodeId, { prompt: newPrompt });
    }
  };

  return (
    <div className={`bg-white border-2 rounded-lg p-4 min-w-[300px] ${selected ? 'border-blue-500' : 'border-gray-200'}`}>
